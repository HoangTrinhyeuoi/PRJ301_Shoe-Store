                            ...
                            <div class="row">
                                ...
                                <div class="col-md-5 mt-4 mt-md-0">
                                    ...
                                    <div class="mb-2 d-flex justify-content-between">
                                        ...
                                        <span>${order.paymentMethod eq 'COD' ? 'Thanh toán khi nhận hàng (COD)' : 'Chuyển khoản ngân hàng'}</span>
                                    </div>
                                    ...
                                    <c:if test="${order.status ne 'CANCELED' && (order.status eq 'PENDING' || order.status eq 'PROCESSING')}">
                                        <div class="mt-3">
                                            <p class="text-muted mb-2"><i class="fas fa-truck me-2"></i>Trạng thái vận chuyển:</p>
                                            <ul class="timeline">
                                                <li>
                                                    <span class="fw-bold text-${order.status eq 'PENDING' || order.status eq 'PROCESSING' || order.status eq 'SHIPPED' || order.status eq 'DELIVERED' ? 'success' : 'muted'}">Đơn hàng đã được xác nhận</span><br>
                                                    <small class="text-muted">
                                                        <c:choose>
                                                            <c:when test="${order.status eq 'PENDING' || order.status eq 'PROCESSING' || order.status eq 'SHIPPED' || order.status eq 'DELIVERED'}">
                                                                <fmt:formatDate value="${order.orderDate}" pattern="dd/MM/yyyy HH:mm" />
                                                            </c:when>
                                                            <c:otherwise>
                                                                Đang chờ xác nhận
                                                ...
                                                <li>
                                                    <span class="fw-bold text-${order.status eq 'PROCESSING' || order.status eq 'SHIPPED' || order.status eq 'DELIVERED' ? 'success' : 'muted'}">Đang chuẩn bị hàng</span><br>
                                                    <small class="text-muted">${order.status eq 'PROCESSING' || order.status eq 'SHIPPED' || order.status eq 'DELIVERED' ? 'Đơn hàng đang được xử lý' : 'Chờ xử lý'}</small>
                                                </li>
                                                <li>
