...
public class CartDAO {
    ...
    public Map<String, String> updateCartItem(int customerId, int productId, int quantity) {
        ...
        try (Connection conn = DBconnection.getConnection()) {
            ...
            // Xử lý đặc biệt nếu số lượng = 0, xóa sản phẩm
            if (quantity <= 0) {
                try (CallableStatement cstmt = conn.prepareCall("{CALL RemoveFromCart(?)}")) {
                    cstmt.setInt(1, cartItemId);
                    cstmt.execute();
    ...
    // Phương thức tăng số lượng sản phẩm trong giỏ hàng
    public Map<String, String> increaseCartItemQuantity(int customerId, int productId) {
        try (Connection conn = DBconnection.getConnection()) {
            // Tìm Cart_ID
            int cartId = getCartId(conn, customerId);
            if (cartId != -1) {
                ...
                if (cartItemId != -1) {
                    ...
                    int currentQuantity = getCurrentQuantity(conn, cartItemId);
                    // Tăng số lượng lên 1
                    return updateCartItem(customerId, productId, currentQuantity + 1);
                }
            }
        } catch (SQLException e) {
            Map<String, String> result = new HashMap<>();
            result.put("status", "error");
            result.put("message", "Lỗi khi tăng số lượng: " + e.getMessage());
            return result;
        }
    ...
    public Map<String, String> decreaseCartItemQuantity(int customerId, int productId) {
        try (Connection conn = DBconnection.getConnection()) {
            // Tìm Cart_ID
            int cartId = getCartId(conn, customerId);
            if (cartId != -1) {
                ...
                if (cartItemId != -1) {
                    // Lấy số lượng hiện tại
                    int currentQuantity = getCurrentQuantity(conn, cartItemId);
                    // Giảm số lượng đi 1, nếu = 0 thì xóa
                    return updateCartItem(customerId, productId, currentQuantity - 1);
                }
            }
        } catch (SQLException e) {
            Map<String, String> result = new HashMap<>();
            result.put("status", "error");
            result.put("message", "Lỗi khi giảm số lượng: " + e.getMessage());
            return result;
        }
    ...
    // Helper method để lấy số lượng hiện tại của một cart item
    private int getCurrentQuantity(Connection conn, int cartItemId) throws SQLException {
        String sql = "SELECT Quantity FROM Cart_Items WHERE Cart_Item_ID = ?";
        try (CallableStatement cstmt = conn.prepareCall(sql)) {
            cstmt.setInt(1, cartItemId);
            ResultSet rs = cstmt.executeQuery();
            if (rs.next()) {
