...
public class ChatBotServlet extends HttpServlet {
    ...
    private String getProductInfo(String userInput) {
        // Kiểm tra xem câu hỏi có phải là về số tiền không
        if (userInput.toLowerCase().contains("có") && userInput.toLowerCase().contains("tiền") && userInput.toLowerCase().contains("mua được những gì")) {
            double budget = extractMoney(userInput);
            if (budget > 0) {
                return getProductsByBudget(budget);
            } else {
                return "❌ Không xác định được số tiền trong câu hỏi.";
            }
        }
    ...
    /**
     * ✅ Hàm tách số tiền từ câu hỏi của người dùng
     */
    private double extractMoney(String userInput) {
        try {
            // Tìm số tiền trong câu hỏi (giả sử số tiền là một số nguyên hoặc số thập phân)
            String[] words = userInput.split("\\s+");
            for (String word : words) {
                // Loại bỏ các ký tự không phải số (như dấu phẩy, ký hiệu tiền tệ)
            ...
            System.out.println("❌ Error in extractMoney: " + e.getMessage());
        }
        return -1; // Trả về -1 nếu không tìm thấy số tiền
    }

    /**
     * ✅ Hàm lấy danh sách sản phẩm có giá nhỏ hơn hoặc bằng số tiền
     */
    private String getProductsByBudget(double budget) {
        ...
        StringBuilder result = new StringBuilder();
        result.append("Với ").append(String.format("%,.0f", budget)).append("₫, bạn có thể mua các sản phẩm sau:\n");
        ...
        try (Connection conn = DBconnection.getConnection();
            ...
            while (rs.next()) {
                ...
                result.append("🔹 **Sản phẩm**: ").append(product).append(" - **Giá**: ")
                      .append(String.format("%,.0f", price)).append("₫\n");
            }

            if (!found) {
                return "❌ Không có sản phẩm nào phù hợp với số tiền " + String.format("%,.0f", budget) + "₫.";
            }
        } catch (SQLException e) {
            e.printStackTrace();
            return "❌ Lỗi khi truy vấn cơ sở dữ liệu.";
